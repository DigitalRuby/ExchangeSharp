...
namespace ExchangeSharp
{
	public sealed partial class ExchangeNDAXAPI : ExchangeAPI
	{
		...
		> OnGetTickersAsync()
		{
			var result = await MakeJsonRequestAsync<Dictionary<string, NDAXTicker>>(
					"ticker",
					"https://core.ndax.io/v1",
					null,
					"GET"
			);
			_marketSymbolToInstrumentIdMapping = result.ToDictionary(
			...
			return result.Select(
					pair =>
							new KeyValuePair<string, ExchangeTicker>(
									pair.Key,
									pair.Value.ToExchangeTicker(Name, pair.Key)
							)
			);
		}
		...
		protected override async Task<ExchangeTicker> OnGetTickerAsync(string symbol)
		{
			return (await GetTickersAsync())
					.Single(
							pair => pair.Key.Equals(symbol, StringComparison.InvariantCultureIgnoreCase)
					)
					.Value;
		}
		...
		{
			var result = await MakeJsonRequestAsync<IEnumerable<NDAXProduct>>(
					"GetProducts",
					null,
					new Dictionary<string, object>() { { "OMSId", 1 } },
					"POST"
			);
			_symbolToProductId = result.ToDictionary(
					product => product.Product,
					product => product.ProductId
			);
			return result.ToDictionary(
					product => product.Product,
					product => product.ToExchangeCurrency()
			);
		}
		...
		{
			var result = await MakeJsonRequestAsync<IEnumerable<Instrument>>(
					"GetInstruments",
					null,
					new Dictionary<string, object>() { { "OMSId", 1 } },
					"POST"
			...
			_marketSymbolToInstrumentIdMapping = result.ToDictionary(
					keySelector: instrument => instrument.Symbol.Replace("_", ""),
		...
		> OnGetCompletedOrderDetailsAsync(string symbol = null, DateTime? afterDate = null)
		{
			...
			if (afterDate.HasValue)
			{
				payload.Add(
						"StartTimeStamp",
						new DateTimeOffset(afterDate.Value).ToUniversalTime().ToUnixTimeMilliseconds()
				);
			}
			if (symbol != null)
			{
			...
			var result = await MakeJsonRequestAsync<IEnumerable<Order>>(
					"GetOrderHistory",
					null,
					payload,
					"POST"
			...
			return result.Select(
					order => order.ToExchangeOrderResult(_marketSymbolToInstrumentIdMapping)
			);
		}
		...
		)
		{
			...
			if (startDate.HasValue)
			{
				payload.Add(
						"StartTimeStamp",
						new DateTimeOffset(startDate.Value).ToUniversalTime().ToUnixTimeMilliseconds()
				);
			}
			if (endDate.HasValue)
			{
				payload.Add(
						"EndTimeStamp",
						new DateTimeOffset(endDate.Value).ToUniversalTime().ToUnixTimeMilliseconds()
				);
			}
			var result = await MakeJsonRequestAsync<IEnumerable<TradeHistory>>(
					"GetTradesHistory",
					null,
					payload,
					"POST"
		...
		{
			var result = await MakeJsonRequestAsync<NDAXDepositInfo>(
					"GetDepositInfo",
					null,
		...
		{
			var result = await MakeJsonRequestAsync<IEnumerable<AccountBalance>>(
					"GetAccountPositions",
					null,
					new Dictionary<string, object>() { { "nonce", await GenerateNonceAsync() } },
					"POST"
			);
		...
		> OnGetAmountsAvailableToTradeAsync()
		{
			...
			return result.ToDictionary(
					balance => balance.ProductSymbol,
					balance => balance.Amount - balance.Hold
			);
		}
		...
		)
		{
			...
			var result = await MakeJsonRequestAsync<SendOrderResponse>(
					"SendOrder",
					null,
					new Dictionary<string, object>()
					{
		...
		)
		{
			...
			var result = await MakeJsonRequestAsync<Order>(
					"GetOrderStatus",
					null,
					new Dictionary<string, object>()
					{
		...
		)
		{
			...
			return result
					.Select(order => order.ToExchangeOrderResult(_marketSymbolToInstrumentIdMapping))
					.Where(
							orderResult =>
									symbol == null
									|| orderResult.MarketSymbol.Equals(
											symbol,
		...
		)
		{
			...
			var result = await MakeJsonRequestAsync<GenericResponse>(
					"CancelOrder",
					null,
					new Dictionary<string, object>()
					{
		...
		)
		{
			...
			var result = await MakeJsonRequestAsync<IEnumerable<IEnumerable<JToken>>>(
					"GetTickerHistory",
					null,
					payload,
					"POST"
		...
		)
		{
			if (PrivateApiKey != null && PublicApiKey != null)
			{
				request.AddHeader(
						"Authorization",
						CryptoUtility.BasicAuthenticationString(
		...
		{
			if (
					base.CanMakeAuthenticatedRequest(payload) && !payload.ContainsKey("skipauthrequest")
			)
			{
		...
		{
			authenticationDetails = await MakeJsonRequestAsync<AuthenticateResult>(
					"Authenticate",
					null,
					new Dictionary<string, object>() { { "skipauthrequest", true } },
					"POST"
			);
		}
		...
		private async Task<long?> GetInstrumentIdFromMarketSymbol(string marketSymbol)
		{
			...
			if (_marketSymbolToInstrumentIdMapping.TryGetValue(marketSymbol, out var value))
			{
				return value;
					...
					_marketSymbolToInstrumentIdMapping.TryGetValue(
		...
		{
			await EnsureInstrumentIdsAvailable();
			var match = _marketSymbolToInstrumentIdMapping.Where(
					pair => pair.Value == instrumentId
			);
		...
		)
		{
			...
			var instrumentIds =
					marketSymbols == null || marketSymbols.Length == 0
							? (await GetMarketSymbolsMetadataAsync())
									.Select(s => (long?)long.Parse(s.AltMarketSymbol))
									.ToArray()
			...
			return await ConnectPublicWebSocketAsync(
					...
					async (socket, bytes) =>
					{
						var messageFrame = JsonConvert.DeserializeObject<MessageFrame>(
											bytes.ToStringFromUTF8().TrimEnd('\0')
									);

						if (
											messageFrame.FunctionName.Equals(
													"SubscribeLevel1",
													StringComparison.InvariantCultureIgnoreCase
											)
											|| messageFrame.FunctionName.Equals(
									...
									)
						{
							...
							if (token["errormsg"] == null)
							{
								...
								var symbol = await GetMarketSymbolFromInstrumentId(
													rawPayload.InstrumentId
											);
								tickers.Invoke(
													new[]
													{
																		new KeyValuePair<string, ExchangeTicker>(
																				symbol,
																				rawPayload.ToExchangeTicker(Name, symbol)
																		),
													}
											);
							}
							...
							await socket.SendMessageAsync(
												new MessageFrame
												{
													...
													Payload = JsonConvert.SerializeObject(
																new { OMSId = 1, InstrumentId = instrumentId, }
														)
												}
										);
						}
		...
		)
		{
			...
			if (marketSymbols == null || marketSymbols.Length == 0)
				instrumentIds = (await GetMarketSymbolsMetadataAsync())
						.Select(s => (long?)long.Parse(s.AltMarketSymbol))
						.ToArray();
			else
			{
							...
							if (messageFrame.Payload != "[]")
							{
								...
								{ // "[[34838,2,0.4656,10879.5,311801351,311801370,1570134695227,1,0,0,0],[34839,2,0.4674,10881.7,311801352,311801370,1570134695227,1,0,0,0]]"
									...
									for (int i = 0; i < jArray.Count; i++)
									{
										...
										var symbol = await GetMarketSymbolFromInstrumentId(
															tradesToken[1].ConvertInvariant<long>()
													);
										var trade = tradesToken.ParseTradeNDAX(
															amountKey: 2,
															priceKey: 3,
															typeKey: 8,
															timestampKey: 6,
