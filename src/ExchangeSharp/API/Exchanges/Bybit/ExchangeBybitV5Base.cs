	...
	public abstract class ExchangeBybitV5Base : ExchangeAPI
	{
		...
		protected override async Task OnGetNonceOffset()
		{
			...
			try
			{
				...
				DateTime serverDate = value.ConvertInvariant<long>().UnixTimeStampToDateTimeNanoseconds();
		...
		protected internal override async Task<IEnumerable<ExchangeMarket>> OnGetMarketSymbolsMetadataAsync()
		{
			...
			var responseToken = await MakeJsonRequestAsync<JToken>($"/v5/market/instruments-info?category={MarketCategory.ToStringLowerInvariant()}");
			...
			foreach (var marketJson in responseToken["list"])
			{
				...
				if (isInverse)
				{
					market.MinTradeSizeInQuoteCurrency = sizeFilter["minOrderQty"].ConvertInvariant<decimal>();
					market.MaxTradeSizeInQuoteCurrency = sizeFilter["maxOrderQty"].ConvertInvariant<decimal>();
				}
				else
				{
		...
		protected override async Task<Dictionary<string, decimal>> OnGetAmountsAvailableToTradeAsync()
		{
			...
			JObject result = await MakeJsonRequestAsync<JObject>("/v5/account/wallet-balance", null, payload);
			...
			if (IsUnifiedAccount.Value)
			{
				...
				amounts.Add("USD", accountBalances["totalAvailableBalance"].ConvertInvariant<decimal>());
			}
		...
		protected override async Task<ExchangeOrderResult> OnPlaceOrderAsync(ExchangeOrderRequest order)
		{
			...
			JToken token = await MakeJsonRequestAsync<JToken>("/v5/order/create", null, payload, "POST");
		...
		public async Task<ExchangeOrderResult> GetRecentOrderAsync(string orderId, string marketSymbol = null, bool isClientOrderId = false, bool? isStop = null)
		{
			if ((MarketCategory == MarketCategory.Linear || MarketCategory == MarketCategory.Inverse) && string.IsNullOrEmpty(marketSymbol))
			{
				throw new ArgumentNullException("marketSymbol is null. For linear & inverse, either symbol or settleCoin is required");
		...
		{
			return await ConnectPrivateWebSocketAsync(url: null,
				messageCallback: (_socket, msg) =>
				{
					JToken token = JToken.Parse(msg.ToStringFromUTF8());
					...
					if (data == null)
					{
						...
						if (op == "auth")
						{
							...
							{
								Logger.Error($"Error authenticating to user data websocket on {Name}: {token["ret_msg"].ToStringInvariant()}");
							}
						}
						else if (op == "subscribe")
						{
							...
							{
								Logger.Error($"Error subscribing to user data websocket on {Name}: {token["ret_msg"].ToStringInvariant()}");
							}
						}
					}
					else
				...
				connectCallback: async (_socket) =>
				{
					long expires = (long)DateTime.Now.UnixTimestampFromDateTimeMilliseconds() + 10000; //10 seconds for auth request
					string stringToSign = $"GET/realtime{expires}";
					string signature = CryptoUtility.SHA256Sign(stringToSign, CryptoUtility.ToUnsecureString(PrivateApiKey));
					var authRequest = new
					{
						op = "auth",
						args = new string[] { CryptoUtility.ToUnsecureString(PublicApiKey), expires.ToStringInvariant(), signature }
					};
		...
		private ExchangeOrderResult ParseOrder(JToken jToken)
		{
			...
			decimal averagePrice = (executedValue == 0 || amountFilled == 0) ? 0m : executedValue / amountFilled;
			...
			ExchangeOrderResult order = new ExchangeOrderResult
			{
				...
				FeesCurrency = marketSymbol.EndsWith("USDT") && MarketCategory == MarketCategory.Linear ? "USDT" : null,
			...
			if (order.Result == ExchangeAPIOrderResult.Filled || order.Result == ExchangeAPIOrderResult.Rejected || order.Result == ExchangeAPIOrderResult.Canceled)
			{
				order.CompletedDate = jToken["updatedTime"] == null ? (DateTime?)null : CryptoUtility.ParseTimestamp(jToken["updatedTime"], TimestampType.UnixMilliseconds);
