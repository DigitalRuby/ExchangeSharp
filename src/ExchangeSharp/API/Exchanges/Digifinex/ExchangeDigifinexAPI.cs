...
namespace ExchangeSharp
{
	public partial class ExchangeDigifinexAPI : ExchangeAPI
	{
		...
		protected override async Task OnGetNonceOffset()
		{
			try
			{
				...
				DateTime serverDate = CryptoUtility.UnixTimeStampToDateTimeSeconds(
						token["server_time"].ConvertInvariant<long>()
				);
		...
		)
		{
			JToken obj = await MakeJsonRequestAsync<JToken>(
					$"/order_book?symbol={marketSymbol}&limit={maxCount}"
			);
			var result = obj.ParseOrderBookFromJTokenArrays(sequence: "date");
			result.LastUpdatedUtc = CryptoUtility.UnixTimeStampToDateTimeSeconds(
					obj["date"].ConvertInvariant<long>()
			);
		...
		{
			JToken obj = await MakeJsonRequestAsync<JToken>(
					$"/trades?symbol={marketSymbol}&limit={limit ?? 500}"
			); // maximum limit = 500
			return obj["data"].Select(
					x =>
							new ExchangeTrade
							{
								...
								Flags =
											x["type"].ToStringLowerInvariant() == "sell"
													? default
													: ExchangeTradeFlags.IsBuy,
							}
			);
		}
		...
		)
		{
			...
			if (periodSeconds <= 60 * 720)
				...
				throw new ArgumentException(
						$"Unsupported periodSeconds: {periodSeconds}",
						"periodSeconds"
				);
			...
			if (startDate != null && endDate != null && limit != null)
				throw new ArgumentException(
						"Cannot specify `startDate`, `endDate` and `limit` all at the same time"
				);
			if (limit != null)
			{
				if (startDate != null)
					...
					x =>
							new ExchangeOrderResult
							{
								...
								OrderDate = CryptoUtility.UnixTimeStampToDateTimeSeconds(
											x["created_date"].ConvertInvariant<long>()
									),
								CompletedDate = CryptoUtility.UnixTimeStampToDateTimeSeconds(
											x["finished_date"].ConvertInvariant<long>()
									),
								Price = x["price"].ConvertInvariant<decimal>(),
			...
			return list.Select(
					x =>
							new ExchangeOrderResult
							{
								...
								TradeDate = CryptoUtility.UnixTimeStampToDateTimeSeconds(
											x["timestamp"].ConvertInvariant<long>()
									),
		...
		protected override async Task<Dictionary<string, decimal>> OnGetAmountsAsync()
		{
			...
			var list = token["list"];
			return list.Where(x => x["total"].ConvertInvariant<decimal>() != 0m)
					.ToDictionary(
							x => x["currency"].ToStringUpperInvariant(),
							x => x["total"].ConvertInvariant<decimal>()
					);
		}
		...
		> OnGetAmountsAvailableToTradeAsync()
		{
			...
			JToken token = await MakeJsonRequestAsync<JToken>("/spot/assets", payload: payload);
			var list = token["list"];
			return list.Where(x => x["free"].ConvertInvariant<decimal>() != 0m)
					.ToDictionary(
							x => x["currency"].ToStringUpperInvariant(),
							x => x["free"].ConvertInvariant<decimal>()
					);
		}
		...
		)
		{
			...
			if (order.IsPostOnly != null)
				payload["post_only"] = order.IsPostOnly.Value ? "1" : "0"; // Default 0, enabled by 1, if enabled the order will be cancelled if it can be executed immediately, making sure there will be no market taking
									...
									await callback.Invoke(
														new KeyValuePair<string, ExchangeTrade>(
																...
																new ExchangeTrade
																{
																	...
																	Timestamp = CryptoUtility
																				.UnixTimeStampToDateTimeSeconds(0)
																				.AddSeconds(
																						trade["time"].ConvertInvariant<double>()
																				),
					...
					(_socket, msg) =>
					{
						...
						if (token["method"].ToStringLowerInvariant() == "depth.update")
						{
							...
							var book = new ExchangeOrderBook
							{
								...
								MarketSymbol = args[2].ToStringUpperInvariant()
							};
							foreach (var x in data["asks"])
							{
								...
								{
									Price = price,
									Amount = x[1].ConvertInvariant<decimal>()
								};
							}
	...
	{
		public const string Digifinex = "Digifinex";
	}
}
